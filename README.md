...
`muted_by` names a field, and a bundle draws every row holding `true` there muted: greyed out, and otherwise a row like any other. It is for rows a table keeps as a record of something that no longer needs attention—a market that has closed for good, a book withdrawn from the collection—so they stay in the table without competing with the rows that are live. A table sets it with `Schema::muted_by`, and it is omitted where a table sets none:

```rust
Schema::new(columns).sortable().muted_by("permanently_closed")
```
...
Each of the five tones is at least 4.5:1 against both the page and a card, in both themes, which is what decides how dark the light theme's are: a brighter green or red than these does not survive a cream background. So is the muted tone, which is also at least 4.5:1 against the raised colour a field turns under the pointer, so the text of a muted row is quieter than the rest and still readable.
